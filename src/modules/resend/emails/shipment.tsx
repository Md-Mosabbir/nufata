import {
  Text,
  Column,
  Container,
  Heading,
  Html,
  Section,
  Tailwind,
  Head,
  Preview,
  Body,
  Row,
  Link,
} from "@react-email/components"
import { CustomerDTO, OrderDTO, FulfillmentDTO } from "@medusajs/framework/types"

type ShipmentCreatedEmailProps = {
  order: OrderDTO & {
    customer: CustomerDTO
    fulfillments?: FulfillmentDTO[]
  }
  isAdmin?: boolean
}

/* ------------------------
   Reusable Components
-------------------------*/

function AdminBanner() {
  return (
    <Section className="bg-blue-500 text-white px-6 py-2 text-center font-bold uppercase tracking-wider">
      🚚 Admin Shipment Notification 🚚
    </Section>
  )
}

function EmailHeader() {
  return (
    <Section className="bg-[#2563eb] text-white px-6 py-4">
      <Heading className="text-xl font-bold tracking-wide">NUFATA'S</Heading>
    </Section>
  )
}

function EmailMessage({ isAdmin, order }: { isAdmin?: boolean; order: OrderDTO }) {
  return (
    <Container className="p-6">
      <Heading className="text-2xl font-bold text-center text-[#2563eb]">
        {isAdmin
          ? `Order #${order.display_id} Has Been Shipped`
          : `Your Order Has Been Shipped!`}
      </Heading>
      <Text className="text-center text-gray-700 mt-2">
        {isAdmin
          ? "A shipment has been created for this order."
          : "Good news! Your order is on its way. Track your shipment below."}
      </Text>
    </Container>
  )
}

function OrderInfo({ order }: { order: OrderDTO }) {
  return (
    <Section className="mt-8">
      <Heading className="text-xl font-semibold text-gray-800 mb-4">
        Order Information
      </Heading>
      <Row className="text-gray-600">
        <Column className="w-1/2">
          <Text className="m-0">Order ID</Text>
        </Column>
        <Column className="w-1/2 text-right">
          <Text className="m-0">#{order.display_id}</Text>
        </Column>
      </Row>
    </Section>
  )
}

function FulfillmentInfo({ fulfillments }: { fulfillments?: FulfillmentDTO[] }) {
  if (!fulfillments || fulfillments.length === 0) return null

  return (
    <Section className="mt-8">
      <Heading className="text-xl font-semibold text-gray-800 mb-4">
        Shipment Details
      </Heading>
      {fulfillments.map((fulfillment, idx) => (
        <Container key={fulfillment.id || idx} className="mb-4 p-4 border rounded">
          <Text>
            <b>Shipped At:</b>{" "}
            {fulfillment.shipped_at
              ? new Date(fulfillment.shipped_at).toLocaleString()
              : "N/A"}
          </Text>
          {fulfillment.labels && fulfillment.labels.length > 0 && (
            <div>
              <Text className="mt-2"><b>Tracking:</b></Text>
              <ul>
                {fulfillment.labels.map((label, i) => (
                  <li key={label.id ?? i}>
                    {label.tracking_number ? (
                      <Link href={label.tracking_url || `https://track.aftership.com/${label.tracking_number}`}>
                        {label.tracking_number}
                      </Link>
                    ) : (
                      "No tracking number"
                    )}
                  </li>
                ))}
              </ul>
            </div>
          )}
        </Container>
      ))}
    </Section>
  )
}

function CustomerInfo({ order }: { order: OrderDTO & { customer: CustomerDTO } }) {
  return (
    <Section className="mt-8">
      <Heading className="text-xl font-semibold text-gray-800 mb-4">
        Customer Information
      </Heading>
      <Row className="text-gray-600">
        <Column className="w-1/2"><Text>Name</Text></Column>
        <Column className="w-1/2 text-right">
          <Text>{order.customer?.first_name} {order.customer?.last_name}</Text>
        </Column>
      </Row>
      <Row className="text-gray-600">
        <Column className="w-1/2"><Text>Email</Text></Column>
        <Column className="w-1/2 text-right"><Text>{order.email}</Text></Column>
      </Row>
    </Section>
  )
}

function EmailFooter({ isAdmin, order }: { isAdmin?: boolean; order: OrderDTO }) {
  return (
    <Section className="bg-[#dbeafe] p-6 mt-10 rounded-b">
      <Text className="text-center text-[#1e40af] text-sm">
        {isAdmin
          ? "This is an automated shipment notification for administrators of NUFATA'S."
          : "If you have any questions, feel free to message us on our Facebook page."}
      </Text>
      <Text className="text-center text-[#1e3a8a] text-sm">
        Order Token: {order.id}
      </Text>
      <Text className="text-center text-[#a3a3a3] text-xs mt-4">
        © {new Date().getFullYear()} NUFATA'S — Homemade Pithas & Desserts from the heart of Bangladesh.
      </Text>
    </Section>
  )
}

/* ------------------------
   Main Component
-------------------------*/

function ShipmentCreatedEmailComponent({ order, isAdmin }: ShipmentCreatedEmailProps) {
  return (
    <Tailwind>
      <Html className="font-sans bg-[#eff6ff]">
        <Head />
        <Preview>
          {isAdmin
            ? `Order #${order.display_id} Has Been Shipped (Admin)`
            : "Your Order from NUFATA'S Has Been Shipped!"}
        </Preview>
        <Body className="bg-white my-10 mx-auto w-full max-w-2xl rounded shadow-md border border-[#60a5fa]">
          {isAdmin && <AdminBanner />}
          <EmailHeader />
          <EmailMessage isAdmin={isAdmin} order={order} />

          <Container className="px-6">
            <OrderInfo order={order} />
            <FulfillmentInfo fulfillments={order.fulfillments} />
            {isAdmin && <CustomerInfo order={order} />}
          </Container>

          <EmailFooter isAdmin={isAdmin} order={order} />
        </Body>
      </Html>
    </Tailwind>
  )
}

export const shipmentCreatedEmail = (props: ShipmentCreatedEmailProps) => (
  <ShipmentCreatedEmailComponent {...props} />
)
